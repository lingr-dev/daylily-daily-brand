...
7. 🔶 **内容录入**（2026-09-16）：文案已全部录入并发布，**图片待图床**（§6）。

   `scripts/seed-content.mts` 同时承担两件事，靠 `--update` 区分：
   默认只补缺的（给空仓库用），`--update` 覆盖已存在文档的 data
   （给「文案由本仓库定稿、要整体刷一遍」用）。后者会盖掉编辑在后台的改动，
   跑之前要清楚这一点。

   富文本里的强调用标记语法写（`**粗体**` / `[[高亮]]`），偏移由脚本算。
   Prismic 的 spans 是按字符下标记的，手数下标太容易错 —— 中文没有词边界，
   改一个字后面全要跟着挪。

   落地页大标题的高亮词（「有**同一个答案**」）走 rich-text 的自定义 label
   `highlight`，不是挪用 `em`：`em` 是「强调」，这里表达的是「品牌高亮」，
   两者在别处会打架。

   **⚠️ 上线前必须由人补齐的**：`/privacy` 与 `/terms` 目前只有占位页
   （正文一句「内容整理中」）。页脚链到这两个路径，静态导出下链到不存在的
   路径就是硬 404，所以先建页把链接接上 —— 但隐私政策与用户协议是法律文本，
   不能由脚本编造。国内主体做了 ICP 备案，这两页的实质内容是合规要求。
8. **图片**：图床对接后回填（§6）。
9. **验证**：
